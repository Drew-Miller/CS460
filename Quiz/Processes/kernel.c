...
PROC *kfork(char *filename) // create a child that begins from body
{
    ...
    if (filename)
    {
	  segment = (p->pid * 0x0800) + 0x1800; // Create the user mode for this process!
      //load the file
...
int fork()
{
  ...
  segment = (p->pid * 0x0800) + 0x1000;
  
  //aka 32k bytes
  //copies 16k words form the running (or parent's) umode into
  //the child's umode
  copyImage(running->uss, segment, 16*0x0100);
