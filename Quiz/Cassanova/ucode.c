...
char *cmd[]={"pid", "ps", "chname", "fork", "switch", 
			"wait", "exit", "kmode", "exec", "hop", 0};
int getc();
void putc(char c);
#define LEN 64
...
{
   printf("***************** Menu *********************************\n");
   printf("*  ps kmode chname  fork  switch  wait hop exec  exit  *\n");
   printf("********************************************************\n");
}
...
{
	int i; u16 seg;
	printf("enter segment to jump to: ");
	i = geti();
	...
	else { printf("segment requested is not allowed\n"); }
}
