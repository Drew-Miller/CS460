...
//to hop to the next process
int khop(u16 segment)
{
  //copies 32k words form the running uMode into the new uMode
  copyImage(running->uss, segment, 32*1024);
  
  //set up the childs uss and usp
  running->uss = segment;
  running->usp = running->usp; //since the images are the same, so is the offset (sp)
  ...
  return running->pid; // return child PROC pointer
}
