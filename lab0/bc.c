...
void printPartition(struct partition *part, int number)
{
		//print out all of the values in the partition
        printf("Partition /dev/sda%d\n", (number + 1));
        printf("BEGIN: CHS = %l %l %l\n", part->cylinder, part->head, part->sector);
        printf("NR_SECTORS: %l\n", part->start_sector);
        //printf("END: CHS = %l %l %l\n\n", part->end_cylinder, part->end_head, part->end_sector);
}
