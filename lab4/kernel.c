...
{
	int i, length = 0;
	char filename[64], test[64], *cp = filename;
	...
	//tokenize here, get file name here, etc.
	//copy /bin to the file name please
	
	//if we cannot load the filename into the segment
	if(!load(filename, segment)) { return -1; }
	...
	if(length % 2 != 0)
	{
		//add an extra buffer to the stack
		length++;
	}
	...
	//2 +   length
	//s -> [cmd a1 .... an]
	//s is also the pointer to the string in umode
	//string should be
	
	
	put_word(segment, segment, -2*12); //saved uds
	put_word(segment, segment, -2*11); //saved eds
	put_word(segment, segment, -2*2);  //uCS = segment; uPC = 0;
	put_word(0x200, segment, -2*1);    //Flag = 00x200. sets the Tracer and Halt bit
}
